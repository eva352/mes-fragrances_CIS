...
Le portail support est exposé derrière Caddy sous :

- Frontend : `https://pilot.auroramind.fr/support`
- API : `https://pilot.auroramind.fr/support/api`

Le contrat d’API de ce document doit donc être lu avec ce préfixe de publication.  
Le backend FastAPI peut rester monté en interne sur `/api`, Caddy se chargeant de l’exposition publique sous `/support/api`.
...
```txt
https://pilot.auroramind.fr/support/api
```
...
```json
{
  ...
  "page_url": "https://pilot.auroramind.fr/support/new",
  "browser_context": {
    "user_agent": "Mozilla/5.0 ...",
    "viewport": "1920x1080"
  },

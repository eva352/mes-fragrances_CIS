...
interface Feature124Props {
  className?: string;
}

const Feature124 = ({ className }: Feature124Props) => {
  return (
    <section
      className={cn(
    ...
    >
      ...
      <div className="relative flex h-[430px] w-full p-4 before:absolute before:inset-0 before:bg-primary/20 before:[mask-image:url(https://deifkwefumgah.cloudfront.net/shadcnblocks/block/patterns/cross-pattern.svg)] before:[mask-size:32px_32px] before:[mask-repeat:repeat]">
        ...
        <div className="relative z-10 m-auto w-[80%]">
          <img
            src="/blocks/features/feature-124/placeholder-1.svg"
            alt="placeholder hero"
            className="max-h-[350px] w-full rounded-md object-cover"
          ></img>
        </div>
        <div className="absolute inset-0 z-5 bg-gradient-to-r from-background/50 via-transparent to-background/50"></div>
        <div className="absolute inset-0 z-5 bg-gradient-to-t from-background/50 via-transparent to-background/50"></div>
      </div>
    </section>
  );
};

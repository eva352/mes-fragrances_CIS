...
export function PerfumeCard({
  ...
  const buttonLabel = ctaLabel || (hasOffers ? "Voir les offres" : "Découvrir le parfum");
  const cardHref = hasOffers ? `${PUBLIC_PATHS.perfume(perfume.slug)}#offres` : PUBLIC_PATHS.perfume(perfume.slug);
  ...
  return (
    <article className="group rounded-[2.1rem] border border-[rgba(154,78,99,0.22)] bg-[rgba(255,255,255,0.62)] p-4 shadow-[0_24px_56px_rgba(168,135,131,0.10)] backdrop-blur-[6px] transition duration-300 hover:-translate-y-1 hover:shadow-[0_32px_72px_rgba(176,138,139,0.14)]">
      <Link
        href={cardHref}
        aria-label={hasOffers ? `Voir les offres pour ${perfume.brand} ${perfume.name}` : `Découvrir ${perfume.brand} ${perfume.name}`}
        className="block rounded-[2rem] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--mf-rose-strong))] focus-visible:ring-offset-2"
      >
        <PerfumeVisual
          name={perfume.name}
          brand={perfume.brand}
      ...
      <dl className="mt-4 grid gap-3 rounded-[1.5rem] border border-[hsla(var(--mf-line),0.8)] bg-[hsla(var(--mf-cream),0.72)] p-4 text-sm">
        ...
        {keyNotes.length ? (
          <div className="space-y-1">
            ...
            <dd className="text-[hsl(var(--mf-ink))]">{keyNotes.join(" · ")}</dd>
          </div>
        ) : null}
      </dl>
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        ...
        >
          <Link href={cardHref}>{buttonLabel}</Link>
        </Button>
      </div>
    </article>
  );
